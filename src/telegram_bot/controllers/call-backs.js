...
export const filterSelectedItem = async (chatId, bot, id) => {
  ...
  if (!list.length) return bot.sendMessage(chatId, title.empty_basket, createItemKeyboardLayout)
  
  // If the "list" array is not empty, send a message to the chat with the "empty_basket" title and call the "createItem" function.
  bot.sendMessage(chatId, title.empty_basket, createItemKeyboardLayout)
  ...
  return bot.sendMessage(chatId, title.item_title, createInlineKeyboard(list))
