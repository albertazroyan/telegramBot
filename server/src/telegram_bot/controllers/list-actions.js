...
export const newlist = async (bot, chatId, text) => {
  try {
    ...
    // Find the user ID from User model using telegram_chat_id
    const user_id = await findID(User, where)
    ...
    const res = await updateOrCreate(List, { name: text, user_id })
...
export const newItem = async (bot, chatId, text) => {
  ...
  try {
    const { id: list_id } = session.session

    const newItem = { name: text, list_id }
